# 実施概要
ポスター記載の「接続先情報・講義ノート」のURLは、現在表示中のGithubのコンテンツです。本講座で学ぶ知識や技能は、メタバースプラットフォームである「Spatial」を用いてメタバース上に構築した街や観光施設・公園などを自動車で走るゲーム開発やメタバース観光の作成に役立ちます。
...
Spatialで提供しているクリエイターツールキットを使うための準備です。クリエイターツールキットを使うことでゲームや仮想オフィスなど様々なコンテンツを作ることができます。

Spatialクリエイターツールキットのサンプル >> https://docs.spatial.io/samples
#### Step1-1a.Spatialアカウントを持っていない場合は、メタバースプラットフォームSpatialのアカウント作成が必要
